<x-app-layout>
    <div class="w-full p-5 bg-white rounded-lg">
        ...
        <div class="">
            <form action="{{ route('beranda') }}" method="GET" id="formPencarian"
                ...
                <div class="w-1/4">
                    <select id="pencarianBuku" name="query" onchange="submitKategori()"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-800 focus:border-green-800 block w-full p-4">
                ...
                <div class="w-1/4">
                    <a href="{{ route('buku.random') }}"
                        class="bg-transparent text-green-700 border-2 border-green-700 hover:bg-green-800 hover:text-white hover:border-transparent font-medium rounded-lg text-sm p-4 text-center me-2 flex items-center gap-2">
                ...
                @foreach ($buku as $item)
                    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow">
                        <a href="{{ route('buku.show', $item->slug) }}">
                            <img class="rounded-t-lg w-[full] h-[245px]" src="{{ asset($item->foto) }}" alt="{{ $item->foto }}" />
                        </a>
                        <div class="p-3">
                            <div>
                                @foreach ($item->kategoriBukuRelasi as $kategoriBuku)
                                    <span
                                        class="px-3 py-2 text-xs font-medium text-center text-white bg-secondary rounded-lg">{{ $kategoriBuku->kategori->nama_kategori }}</span>
                                    <span
                                        class="px-3 py-2 text-xs font-medium text-center text-white bg-secondary rounded-lg">{{ $item->stok }}</span>
                                @endforeach
                            </div>
                            <p class="mb-3 mt-3 text-sm font-semibold text-gray-900">{{ $item->judul }}</p>
