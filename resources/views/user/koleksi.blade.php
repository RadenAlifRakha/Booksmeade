        ...
        <div class="flex justify-between items-center ">
            ...
            <div>
                <button type="button" data-modal-target="modalTambah" data-modal-toggle="modalTambah"
                    class="text-white font-bold bg-green-800 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 mb-2">
                    <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M5 12h14m-7 7V5" />
                    </svg>Tambah buku yang dipinjam
                </button>
            </div>
        </div>
        <div class="flex space-x-0 items-center">
            <hr class="border-2 border-gray-900 cursor-pointer w-[155px]">
                ...
                <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow">
                    ...
                    <div class="p-5">
                        @foreach ($item->buku->kategoriBukuRelasi as $kategori)
                            <a href=""
                                class="px-3 py-2 text-xs font-medium text-center text-white bg-secondary rounded-lg">{{ $kategori->kategori->nama_kategori }}</a>
                        @endforeach
                        ...
                        <div class="flex justify-between items-center">
                            <h5 class="mb-2 text-sm font-semibold tracking-tight text-gray-900">
                                {{ $item->buku->penulis }}</h5>
                            <div>
                                <button id="dropdownMenuIconHorizontalButton"
                                    data-dropdown-toggle="opsiKoleksi{{ $index }}"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <form action="{{ route('koleksi.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <button type="submit"
                    class="text-white bg-green-800 hover:bg-green-500 focus:ring-2 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-5">
                    Simpan
                </button>
            </form>
        </div>
